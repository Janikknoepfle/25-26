...
Wir wollen zuerst einen Blick auf die Anzahl an Erwerbstätigen und Einwohnern in Deutschland werfen. Hier haben wir 42,1 Mio. Erwerbstätige und 78,1 Mio. Einwohner in Deutschland. Dies sollte stimmen, da wir Hamburg (1,8 Mio.), Berlin (3,8 Mio.) und Bremen (0,6 Mio.), sowie Bremerhaven (0,1 Mio.) nicht in unserem Datensatz haben. Die Daten wollen wir uns als einfachen Tibble Datensatz anzeigen lassen:
...
Im Durchschnitt liegt die Arbeitslosenquote in 2023 bei 5,29 Prozent. Dies mag uns zuerst etwas hoch erscheinen, jedoch sollten wir bedenken, dass wir **alle** Arbeitslosen mit in unsere Analyse einbezogen haben, d.h. Bezieher von SGB II und SGB III. Ein kurzer Konsistenzcheck auf [Statista](https://de.statista.com/statistik/daten/studie/1224/umfrage/arbeitslosenquote-in-deutschland-seit-1995/) zeigt uns die dort gemeldete Arbeitslosenquote von 5.7% für 2023. Unsere niedrigere Quote könnte insbesondere daran liegen, dass die Großstädte Berlin und Hamburg nicht in unserer Analyse enthalten sind.
Die Standardabweichung beträgt 2,08 und zeigt damit, dass es in Deutschland deutliche regionale Unterschiede bzgl. der Arbeitslosenquote gibt. Ein Blick auf die Verteilung zeigt, dass der Landkreis mit der geringsten Arbeitslosenquote nur eine Arbeitslosenquote von 1,64% (`p0` in der obigen Tabelle) aufweist und der Landkreis mit der höchsten Arbeitslosenquote von 14,2% (`p100` in der obigen Tabelle). Zwar sind die Werte noch ein ganzes Stück von dem Durchschnitt der Arbeitslosenquote in Spanien entfernt ([fast 12,2% im Jahr 2023](https://de.statista.com/statistik/daten/studie/17327/umfrage/arbeitslosenquote-in-spanien/)), zeigen jedoch schon, dass es auch in Deutschland durchaus Regionen mit einer sehr hohen Arbeitslosenquote gibt.

**Verschuldung pro Kopf**

Bei der Verschuldung der Landkreise ergibt sich ein ähnliches Bild. Durchschnittlich beträgt die Verschuldung der Landkreise 3052€, mit einer Standardabweichung von 2303€. D.h. auch hier gibt es eine große Bandbreite bzgl. der Verschuldung einzelner Landkreise.
...
Nachdem wir im vorherigen Abschnitt bereits gesehen haben, dass es wohl deutliche regionale Unterschiede bei allen Variablen geben muss, wollen wir uns nun noch die Arbeitslosenquote auf Bundeslandebene anschauen. Erinnern wir uns daran das wir hier Querschnittsdaten für das Jahr 2023 für alle deutschen Landkreise betrachten. Weiterhin hat uns die vorherige Tabelle bereits gezeigt, dass wir mit "missing Values" rechnen müssen (`n_missing` bei der skimr-Tabelle). Um nun einen ersten Überblick über die regionale Verteilung der Arbeitsenquote in 2023 zu gewinnen schauen wir uns die Arbeitslosenquote auf Bundeslandebene an, aggregiert über die Landkreise eines Bundeslandes. Hierzu wollen wir uns des Pakets `kableExtra` bedienen um schönere Tabellen zu erstellen:
...
bula_data |>
  ...
  row_spec(c(8,9, 10, 12,13), bold = T, color = "white", background = "#BBBBBB") |>
...
Durch diese Aufteilung treten die Unterschiede in der Arbeitslosenquote zwischen den ehemaligen ost- und westdeutschen Landkreisen besonders stark zutage. Insbesondere wenn wir uns die Quantile anschauen: Im **25% Quantil** in **Ostdeutschland** ist die Arbeitslosenquote bei **5,69%**, in **Westdeutschland** ist das **75% Quantil** bei einer Arbeitslosenquote von **6,17%**!
...
Nun haben wir uns ein erstes Bild über die Daten, insbesondere die Arbeitslosigkeit in 2023 machen können.
...
Die Variable, welche uns besonders interessiert ist die Arbeitslosenquote, insbesondere da ihr Cousin gemeint hat, dass Deutschland kein Problem mit der Arbeitslosigkeit hat. Wir wollen hier alle Datenpunkte zeigen, d.h. die Arbeitslosenquote eines jeden Landkreises für das Jahr 2023, getrennt nach Ost- und Westdeutschland. Weiterhin wollen wir unsere Grafik um einen Boxplot erweitern um einen Vergleich des Medians der Arbeitslosenquote in Ost- und Westdeutschland zu ermöglichen.
...
alo_quote_jitter <- gesamtdaten |>
  ...
  labs(title = "Arbeitslosenquote in Deutschland",
       ...
       x = "",
       y = "Arbeitslosenquote",
       caption = "Quelle: Daten der Agentur für Arbeit aus dem Jahr 2023")
...
alo_quote_jitter_plotly <- gesamtdaten |>
  ...
  labs(title = "Arbeitslosenquote in Deutschland",
       ...
       x = "",
       y = "Arbeitslosenquote",
       caption = "Quelle: Daten der Bundesagentur Agentur für Arbeit aus dem Jahr 2023.") +
...
Wir haben im vorherigen Abschnitt gesehen, dass es durchaus deutliche regionale Unterschiede in der Arbeitslosenquote in 2023 gibt. Doch was könnten die Treiber dafür sein? 
...
bip_zeitreihe_namen |>
  group_by(ost_name, Jahr) |>
  summarise(durchschnitt = mean(bip_pro_kopf, na.rm = TRUE), .groups = "drop") |>
  ggplot(aes(x = Jahr, y = durchschnitt, color = ost_name)) +
  geom_line(linewidth = 0.6) +
  scale_color_manual(values = c("#D55E00", "#0072B2")) +
  scale_y_continuous(
    trans = "log10",
    breaks = c(5000, 10000, 15000, 20000, 25000, 30000, 35000, 40000, 45000),
    limits = c(5000, 50000)
  ) +
  ...
  theme(legend.position = "none") +
  labs(
    title = "Ein Vergleich des BIP pro Kopf von ost- und westdeutschen Landkreisen",
  ...
  annotate(
    "text",
    x = 2000, y = 15000, label = "Durchschnitt für ostdeutsche Landkreise",
    color = "#D55E00", hjust = 0, size = 3
  ) +
  annotate(
    "curve",
    x = 2007, xend = 2005, y = 15500, yend = 18500,
    curvature = 0.3, color = "#D55E00", linewidth = 0.1
  ) +
  annotate(
    "text",
    x = 1991, y = 31000, label = "Durchschnitt für westdeutsche Landkreise",
    color = "#0072B2", hjust = 0, size = 3
  ) +
  annotate(
    "curve",
    x = 1995, xend = 1997, y = 29000, yend = 25000,
    curvature = 0.2, color = "#0072B2", linewidth = 0.1
  )
```
...
base <- bip_wachstum |>
  filter(Jahr >= 2000) |>
  filter(!is.na(bip_pro_kopf_wachstum))

avg <- base |>
  group_by(ost_name, Jahr) |>
  summarise(durchschnitt = mean(bip_pro_kopf_wachstum, na.rm = TRUE), .groups = "drop")

ggplot() +
  # alle Landkreise (grau)
  geom_line(
    data = base,
    aes(x = Jahr, y = bip_pro_kopf_wachstum, group = Regionalschluessel),
    color = "grey", linewidth = 0.2, alpha = 0.6, na.rm = TRUE
  ) +
  # Durchschnittslinien ost/west
  geom_line(
    data = avg,
    aes(x = Jahr, y = durchschnitt, color = ost_name),
    linewidth = 0.9, na.rm = TRUE
  ) +
  scale_color_manual(values = c("#D55E00", "#0072B2")) +
  theme_minimal() +
  labs(
    color = "Durchschnitt der Landkreise",
    title = "Die Wachstumsrate des BIP pro Kopf von\nost- und westdeutschen Landkreisen",
    caption = "Quelle: Daten der Statistischen Ämter der Länder und des Bundes.",
    x = "Jahr",
    y = "Veränderung des BIP pro Kopf"
  ) +
  theme(legend.position = "none") +
  # feste Labels: annotate statt aes()
  annotate(
    "text", x = 2000, y = 40,
    label = "-- Durchschnitt für ostdeutsche Landkreise",
    color = "#D55E00", hjust = 0, size = 4
  ) +
  annotate(
    "text", x = 2000, y = 35,
    label = "-- Durchschnitt für westdeutsche Landkreise",
    color = "#0072B2", hjust = 0, size = 4
  )
...
gesamtdaten |>
  ...
  theme_bw() +
  labs(title = "Verteilung des BIP pro Kopf für Ost- und\nWestdeutschland",
...
plot_schulden_lk <- ggplot(
  ...
  labs(x = NULL,
       ...
       subtitle = "Öffentliche Schulden pro Kopf in 2023") +
  theme_void()
...
plot_schulden_lk <- ggplot(
  ...
  labs(x = NULL,
       y = NULL,
       title = "Wie verschuldet sind die deutschen Landkreise?",
       subtitle = "Öffentliche Schulden im Verhältnis zum BIP in 2023") +
  theme_void()
...
plot_alo_lk <- ggplot(
  ...
  labs(x = NULL,
       ...
       subtitle = "Dargestellt ist die Arbeitslosenquote für alle Landkreise in 2023") +
  theme_void()
...
Zuerst wollen wir das zuvor berechnete BIP-Wachstum auf das Jahr 2022 einschränken und dies zu unseren Gesamtdaten hinzufügen.
...
Anhand der Punktewolke könnten wir abschätzen, dass der Zusammenhang zwischen der Arbeitslosenquote und dem BIP-pro-Kopf-Wachstum nahe Null sein müsste. D.h. wir würden vermuten das es keinen Zusammenhang zwischen der BIP-pro-Kopf Wachstumsrate und der Arbeitslosenquote eines Landkreises in 2023 gibt. 
